...
async function getUserWithRoleByEmail(email: string): Promise<UserWithRole | null> {
...
export class AuthService {
  ...
  async login(loginData: LoginRequest) {
    ...
    if (user.isGoogle) {
      throw createError(
        "This account is registered with Google. Please use 'Continue with Google' to login.",
        400,
      );
    }
  ...
  async googleLogin(loginData: GoogleLoginRequest) {
    ...
    if (!user.isGoogle) {
      throw createError(
        "This account was created with email/password. Please use regular login.",
        400,
      );
    }
