...
import { useCurrentClinicDashboardStats } from "@/app/appointments/use-appointments";
...
const ClinicDashboardPage = () => {
  const { data: stats, isLoading: statsLoading, error: statsError } = useCurrentClinicDashboardStats();

  if (statsLoading) {
    return (
      <div className="flex min-h-[40vh] items-center justify-center">
        <Loader title="Loading dashboard" description="Fetching clinic stats..." />
      </div>
    );
  }

  if (statsError || !stats) {
    const message = statsError ? getErrorMessage(statsError) : "Unable to load clinic dashboard stats.";
    const isProfileMissing =
      typeof message === "string" &&
      (message.toLowerCase().includes("clinic profile not found") || message.toLowerCase().includes("not found"));
      ...
      <div className="flex min-h-[40vh] flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        {isProfileMissing ? (
          <Button asChild variant="default">
            <Link href="/onboarding/clinic">Complete clinic onboarding</Link>
          </Button>
        ) : null}
      </div>
    );
  ...
  return (
    <>
      <PageHeader title="Dashboard" description={stats.clinicName} icon={Building2} />
      ...
      <Card className={cardSectionClass()}>
        <CardHeader>
          ...
          <CardDescription>Current clinic appointment distribution.</CardDescription>
        </CardHeader>
