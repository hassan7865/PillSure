...
import { PatientOnboardingRequest, DoctorOnboardingRequest, HospitalOnboardingRequest, ClinicOnboardingRequest, ManufacturerOnboardingRequest, MedicalStoreOnboardingRequest } from '../_components/_types';
...
export const useClinicOnboarding = () => {
  const { showSuccess, showError } = useCustomToast();
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  ...
  const mutate = useCallback(async (data: ClinicOnboardingRequest) => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await onboardingApi.saveClinicOnboarding(data);
      handleRedirect(response, getPostAuthHomeByRole('clinic'), 'Clinic onboarding completed');
      return response;
    } catch (err: any) {
      const errorMsg = getErrorMessage(err);
      setError(err instanceof Error ? err : new Error(errorMsg));
      showError('Failed to save clinic data', errorMsg);
      throw err;
    } finally {
      setIsLoading(false);
    }
...
export const useManufacturerOnboarding = () => {
  const { showSuccess, showError } = useCustomToast();
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  ...
  const mutate = useCallback(async (data: ManufacturerOnboardingRequest) => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await onboardingApi.saveManufacturerOnboarding(data);
      handleRedirect(response, getPostAuthHomeByRole('manufacturer'), 'Manufacturer onboarding completed');
      return response;
    } catch (err: any) {
      const errorMsg = getErrorMessage(err);
      setError(err instanceof Error ? err : new Error(errorMsg));
      showError('Failed to save manufacturer data', errorMsg);
      throw err;
    } finally {
      setIsLoading(false);
    }
...
export const useMedicalStoreOnboarding = () => {
  const { showSuccess, showError } = useCustomToast();
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  ...
  const mutate = useCallback(async (data: MedicalStoreOnboardingRequest) => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await onboardingApi.saveMedicalStoreOnboarding(data);
      handleRedirect(response, getPostAuthHomeByRole('medical_store'), 'Medical store onboarding completed');
      return response;
    } catch (err: any) {
      const errorMsg = getErrorMessage(err);
      setError(err instanceof Error ? err : new Error(errorMsg));
      showError('Failed to save medical store data', errorMsg);
      throw err;
    } finally {
      setIsLoading(false);
    }
...
export const useGetClinicOnboarding = () => {
  const [data, setData] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<Error | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchOnboarding = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const result = await onboardingApi.getClinicOnboarding();
        if (isMounted) {
          setData(result);
        }
      } catch (err) {
        if (isMounted) {
          setError(err instanceof Error ? err : new Error('Failed to fetch clinic onboarding'));
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
...
export const useGetManufacturerOnboarding = () => {
  const [data, setData] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<Error | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchOnboarding = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const result = await onboardingApi.getManufacturerOnboarding();
        if (isMounted) {
          setData(result);
        }
      } catch (err) {
        if (isMounted) {
          setError(err instanceof Error ? err : new Error('Failed to fetch manufacturer onboarding'));
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
...
export const useGetMedicalStoreOnboarding = () => {
  const [data, setData] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<Error | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchOnboarding = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const result = await onboardingApi.getMedicalStoreOnboarding();
        if (isMounted) {
          setData(result);
        }
      } catch (err) {
        if (isMounted) {
          setError(err instanceof Error ? err : new Error('Failed to fetch medical store onboarding'));
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
